...
if user_input := st.chat_input("What do you want to know?"):
    ...
    # Determine if the input is a request for data analysis and checkbox is selected
    if model:
        try:
            if (
                st.session_state.uploaded_data is not None
                and analyze_data_checkbox
                and ("analyze" in user_input.lower() or "insight" in user_input.lower())
            ):
                # Describe the uploaded data for the AI model
